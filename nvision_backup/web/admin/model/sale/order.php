...
class ModelSaleOrder extends Model {
  ...
  public function addOrderSimple($data) {
          ...
          standard = '" . $this->db->escape($data['standard']) . "', 
          need_assistance = '" . (int)$data['need_assistance'] . "', 
          order_status_id = '1', 
  ...
  public function editOrder($order_id, $data) {
   ...
   public function editOrderSimple($order_id, $data) {
        ...
        standard = '" . $this->db->escape($data['standard']) . "', 
        need_assistance = '" . (int)$data['need_assistance'] . "', 
        comment = '" . $this->db->escape($data['comment']) . "', order_status_id = '" . (int)$data['order_status_id'] . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'standard'                   => $order_query->row['standard'],
				'need_assistance'                   => $order_query->row['need_assistance'],
	...
	public function addOrderHistory($order_id, $data) {
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			$subject = sprintf($language->get('text_subject'), $this->config->get('config_name'), $order_id);
