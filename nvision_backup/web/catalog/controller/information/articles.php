	...
	public function index() {
		...
		if ($articles_info) {
			...
			$this->response->setOutput($this->render());
				...
				$pagination->url = $this->url->link('information/articles', $url . '&page={page}', 'SSL');
				
				$this->data['pagination'] = $pagination->render();		
		
	  		$articles_data = $this->model_catalog_articles->getArticles($data);
	  		...
	  		if ($articles_data) {
				...
				foreach ($articles_data as $result) {
					$this->data['articles_data'][] = array(
						...
						'thumb'				=> $this->model_tool_image->resize($result['image'], $this->config->get('articles_thumb_width'), $this->config->get('articles_thumb_height')),
					);
				}
